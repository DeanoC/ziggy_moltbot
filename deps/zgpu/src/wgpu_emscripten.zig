...
pub const VertexBufferLayout = extern struct {
    // Matches `webgpu.h` (emdawnwebgpu) layout: nextInChain, stepMode, arrayStride, ...
    next_in_chain: ?*const ChainedStruct = null,
...
pub const Device = *opaque {
    ...
    pub fn createBindGroupLayout(device: Device, descriptor: BindGroupLayoutDescriptor) BindGroupLayout {
        ...
        const entries_c = std.heap.c_allocator.alloc(c.WGPUBindGroupLayoutEntry, count) catch unreachable;
        ...
        if (count > 0) {
            ...
            for (entries_c, 0..) |*dst, i| {
                ...
                // Emscripten's WebGPU glue treats these as a union: only one of
                // buffer/sampler/texture/storageTexture should be present.
                //
                // If we leave the unused members as `Undefined`, the JS glue can
                // mistakenly consider them "in use" and then throw due to missing
                // required fields (e.g. storageTexture.format).
                const Kind = enum { none, buffer, sampler, texture, storage_texture };
                var kind: Kind = .none;
                var kind_count: u32 = 0;
                if (e.buffer.binding_type != .undef) {
                    kind = .buffer;
                    kind_count += 1;
                }
                if (e.sampler.binding_type != .undef) {
                    kind = .sampler;
                    kind_count += 1;
                }
                if (e.texture.sample_type != .undef) {
                    kind = .texture;
                    kind_count += 1;
                }
                if (e.storage_texture.access != .undef or e.storage_texture.format != .undef) {
                    kind = .storage_texture;
                    kind_count += 1;
                }

                // Most callers should set exactly one binding type per entry.
                // In release builds we still normalize to "none" if unspecified, and
                // pick the last one if multiple are (incorrectly) set.
                if (kind_count != 1) {
                    if (builtin.mode == .Debug) {
                        std.debug.panic("BindGroupLayoutEntry must specify exactly one binding kind (got {d})", .{kind_count});
                    }
                    if (kind_count == 0) kind = .none;
                }

                const use_buffer = kind == .buffer;
                const use_sampler = kind == .sampler;
                const use_texture = kind == .texture;
                const use_storage_texture = kind == .storage_texture;
                ...
                dst.* = .{
                    ...
                    .buffer = .{
                        .nextInChain = null,
                        .type = if (use_buffer)
                            @intCast(@intFromEnum(e.buffer.binding_type))
                        else
                            @intCast(c.WGPUBufferBindingType_BindingNotUsed),
                    ...
                    .sampler = .{
                        .nextInChain = null,
                        .type = if (use_sampler)
                            @intCast(@intFromEnum(e.sampler.binding_type))
                        else
                            @intCast(c.WGPUSamplerBindingType_BindingNotUsed),
                    },
                    .texture = .{
                        .nextInChain = null,
                        .sampleType = if (use_texture)
                            @intCast(@intFromEnum(e.texture.sample_type))
                        else
                            @intCast(c.WGPUTextureSampleType_BindingNotUsed),
                        .viewDimension = if (use_texture)
                            @intCast(@intFromEnum(e.texture.view_dimension))
                        else
                            @intCast(c.WGPUTextureViewDimension_Undefined),
                        .multisampled = if (e.texture.multisampled) 1 else 0,
                    },
                    .storageTexture = .{
                        .nextInChain = null,
                        .access = if (use_storage_texture)
                            @intCast(@intFromEnum(e.storage_texture.access))
                        else
                            @intCast(c.WGPUStorageTextureAccess_BindingNotUsed),
                        .format = if (use_storage_texture)
                            @intCast(@intFromEnum(e.storage_texture.format))
                        else
                            @intCast(c.WGPUTextureFormat_Undefined),
                        .viewDimension = if (use_storage_texture)
                            @intCast(@intFromEnum(e.storage_texture.view_dimension))
                        else
                            @intCast(c.WGPUTextureViewDimension_Undefined),
                    },
                };
...
pub const SwapChain = struct {
    ...
    pub fn getCurrentTextureView(sc: *SwapChain) ?TextureView {
        ...
        sc.acquired = @ptrCast(st.texture orelse return null);
        return sc.acquired.?.createView(.{});
    }

    pub fn present(sc: *SwapChain) void {
        // Emscripten's `emdawnwebgpu` WebGPU implementation presents implicitly after
        // queue submission. `wgpuSurfacePresent` is a stub that aborts.
        if (sc.acquired) |t| {
            t.release();
            sc.acquired = null;
        }
