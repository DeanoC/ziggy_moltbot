...
java_jars: std.ArrayListUnmanaged(LazyPath),
resources: std.ArrayListUnmanaged(Resource),
native_lib_files: std.ArrayListUnmanaged(NativeLibFile),

pub const NativeLibFile = struct {
    /// Prebuilt shared library to package into the APK under `lib/{abi}/{dest_name}`.
    file: LazyPath,
    /// Android ABI directory name (e.g. "arm64-v8a", "x86_64").
    abi: []const u8,
    /// Destination filename (e.g. "libSDL3.so").
    dest_name: []const u8,
};
...
/// Add a prebuilt Java library jar to be dexed and packaged into the APK.
/// This is useful when consuming Android AARs that ship `classes.jar` instead
/// of raw `.java` sources.
pub fn addJavaLibraryJar(apk: *Apk, jar: LazyPath) void {
    const b = apk.b;
    apk.java_jars.append(b.allocator, jar.dupe(b)) catch @panic("OOM");
}

pub const AddNativeLibraryFileOptions = struct {
    file: LazyPath,
    abi: []const u8,
    dest_name: []const u8,
};

/// Add a prebuilt shared library to be packaged into the APK.
pub fn addNativeLibraryFile(apk: *Apk, options: AddNativeLibraryFileOptions) void {
    const b = apk.b;
    apk.native_lib_files.append(b.allocator, .{
        .file = options.file.dupe(b),
        .abi = b.dupe(options.abi),
        .dest_name = b.dupe(options.dest_name),
    }) catch @panic("OOM");
}
...
fn doInstallApk(apk: *Apk) std.mem.Allocator.Error!*Step.InstallFile {
    ...
    // Add prebuilt shared libs (e.g. from AARs) into lib/{abi}/
    for (apk.native_lib_files.items) |native_lib| {
        _ = apk_files.addCopyFile(
            native_lib.file,
            b.fmt("lib/{s}/{s}", .{ native_lib.abi, native_lib.dest_name }),
        );
    }

    // Add *.jar files
    // - Even if java_files.items.len == 0, we still always add the root_jar
    if (apk.java_files.items.len > 0 or apk.java_jars.items.len > 0) {
        var java_classes_output_dir: ?LazyPath = null;
        ...
        if (apk.java_files.items.len > 0) {
            ...
            javac_cmd.addArg("-d");
            java_classes_output_dir = javac_cmd.addOutputDirectoryArg("android_classes");
        ...
        // Add compiled classes from javac (if any) and any prebuilt jar libs.
        if (java_classes_output_dir) |classes_dir| {
            // add each output *.class file
            D8Glob.create(b, d8, classes_dir);
        }
        for (apk.java_jars.items) |jar| {
            d8.addFileArg(jar);
        }
