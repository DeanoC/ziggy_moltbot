...
pub const TextEditor = struct {
    ...
    pub fn draw(
        ...
        const padding = style.frame_padding;
        ...
        const wrap_width = @max(4.0, text_rect.size()[0]);
        const line_height = zgui.getTextLineHeightWithSpacing();

        var lines = buildLines(allocator, self.buffer.items, wrap_width);
        defer lines.deinit(allocator);

        handleMouse(self, queue, rect, text_rect, &lines);
        const text_changed = handleInput(self, allocator, queue, &lines, opts, &action);

        if (text_changed) {
            lines.clearRetainingCapacity();
            buildLinesInto(allocator, self.buffer.items, wrap_width, &lines);
        }
        ...
        const caret_pos = caretPosition(self.buffer.items, &lines, self.cursor, line_height, ctx);
        ...
        drawSelection(ctx, text_rect, &lines, self.buffer.items, self.selectionRange(), line_height, self.scroll_y, t);
        drawText(ctx, text_rect, &lines, self.buffer.items, line_height, self.scroll_y, t);
        if (self.focused) {
            drawCaret(ctx, text_rect, caret_pos, line_height, self.scroll_y, t);
        }

        if (self.focused and !opts.read_only) {
            const ime_pos = .{
                text_rect.min[0] + caret_pos[0],
                text_rect.min[1] + caret_pos[1] - self.scroll_y,
            };
            imgui_bridge.setWantTextInput(true);
            imgui_bridge.setImeData(ime_pos, line_height, true);
        } else {
            imgui_bridge.setWantTextInput(false);
        }
...
fn drawText(
    ...
    for (lines.items, 0..) |line, idx| {
        ...
        ctx.drawText(slice, .{ text_rect.min[0], y }, .{ .color = t.colors.text_primary });
    }
}
...
fn drawSelection(
    ...
    for (lines.items, 0..) |line, idx| {
        ...
        const rect = draw_context.Rect{
            .min = .{ text_rect.min[0] + left, y },
            .max = .{ text_rect.min[0] + right, y + line_height },
        };
...
fn drawCaret(
    ...
    const x = text_rect.min[0] + caret_pos[0];
        ...
        switch (evt) {
            .mouse_down => |md| {
                ...
                if (inside) {
                    ...
                    const local = .{ mouse[0] - text_rect.min[0], mouse[1] - text_rect.min[1] };
                    editor.cursor = cursorFromPosition(editor.buffer.items, lines, local, editor.scroll_y);
...
fn pasteClipboard(editor: *TextEditor, allocator: std.mem.Allocator) void {
    const text_z = zgui.getClipboardText();
...
fn caretPosition(text: []const u8, lines: *std.ArrayList(Line), cursor: usize, line_height: f32, ctx: *draw_context.DrawContext) [2]f32 {
    if (lines.items.len == 0) return .{ 0.0, 0.0 };
    if (lineForIndex(lines, cursor)) |info| {
        const slice = text[info.line.start..cursor];
        const width = textWidth(ctx, slice);
    ...
    const width = textWidth(ctx, text[last_line.start..last_line.end]);
...
fn cursorFromPosition(
    ...
    return indexAtX(text, line, x);
}

fn indexAtX(text: []const u8, line: Line, x: f32) usize {
    var idx = line.start;
    var cur_x: f32 = 0.0;
    while (idx < line.end) {
...
fn buildLines(allocator: std.mem.Allocator, text: []const u8, wrap_width: f32) std.ArrayList(Line) {
