...
pub const DeviceIdentity = struct {
    ...
    pub fn deinit(self: *DeviceIdentity, allocator: std.mem.Allocator) void {
        ...
        if (self.token_role) |role| {
            allocator.free(role);
        }
        if (self.token_scopes) |scopes| {
            freeScopes(allocator, scopes);
        }
    }
};
...
pub fn loadOrCreate(allocator: std.mem.Allocator, path: []const u8) !DeviceIdentity {
    if (try loadIdentity(allocator, path)) |identity| {
        return identity;
    }
    ...
    try saveIdentity(allocator, path, key_pair, device_id, public_key_b64, null, null, null, null);
...
fn loadIdentity(allocator: std.mem.Allocator, path: []const u8) !?DeviceIdentity {
    ...
    var parsed = std.json.parseFromSlice(StoredIdentity, allocator, data, .{}) catch return null;
    defer parsed.deinit();
    ...
    if (parsed.value.device_id == null or !std.mem.eql(u8, parsed.value.device_id.?, device_id)) {
        try saveIdentity(
            allocator,
            path,
            key_pair,
...
fn updateDeviceToken(
    ...
    identity.token_scopes = if (scopes) |value| try dupScopes(allocator, value) else null;
...
fn dupScopes(allocator: std.mem.Allocator, scopes: []const []const u8) ![]const []const u8 {
    ...
    for (scopes, 0..) |scope, idx| {
        out[idx] = try allocator.dupe(u8, scope);
    }
    return out;
}
...
fn deriveDeviceId(allocator: std.mem.Allocator, public_key_raw: []const u8) ![]u8 {
    var digest: [Sha256.digest_length]u8 = undefined;
    Sha256.hash(public_key_raw, &digest, .{});
